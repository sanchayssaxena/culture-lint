...
export default function OnboardingModal({ onComplete }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Step 1: Update Clerk publicMetadata directly from frontend
      // This uses Clerk's built-in updateUser which doesn't need our API
      await user.update({
        unsafeMetadata: { nationality },
      })

      // Step 2: Sync to Neon DB via our API
      try {
        ...
        // DB sync failed but Clerk metadata is saved — not a blocker
        console.warn('DB sync failed, continuing:', apiErr)
      }

      // Step 3: Reload user to get fresh metadata
      await user.reload()
        ...
        <div className="px-6 py-6">
          ...
          <div className="space-y-3 mb-5">
            {[
              ...
              <div key={label}>
                <label className="block text-xs font-bold tracking-widest uppercase mb-1.5" style={{ color: '#666' }}>
                  {label}
                </label>
                <div className="w-full px-3 py-2.5 text-sm font-medium"
          ...
          <form onSubmit={handleSubmit}>
            ...
            {error && (
              <p className="text-xs mb-4 px-3 py-2"
            ...
            <button
              ...
              onMouseEnter={e => { if (nationality && !saving) e.target.style.background = '#7B00C2' }}
              onMouseLeave={e => { if (nationality && !saving) e.target.style.background = '#A100FF' }}
            >
